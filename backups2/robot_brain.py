...
def reach_for_coordinate(x, y, z, speed=800):
    ...
    try:
        ...
        for id, pos in new_p.items():
            move_servo(id, pos, speed)
...
class RobotTuner:
    def _toggle_tracking_timeout_clicked(self):
        # Toggle the TRACKING_TIMEOUT_ENABLED config value
        current = bool(getattr(config, "TRACKING_TIMEOUT_ENABLED", True))
        new_value = not current
        setattr(config, "TRACKING_TIMEOUT_ENABLED", new_value)
        # Update the GUI label if present
        if hasattr(self, "tracking_timeout_var"):
            secs = int(float(getattr(config, "FLAGPOLE_TIMEOUT", 300.0)))
            self.tracking_timeout_var.set(
                f"Tracking Timeout: {'ON' if new_value else 'OFF'} ({secs}s)"
            )
        print(f"Tracking timeout toggled to: {'ON' if new_value else 'OFF'}")
    def __init__(self):
        self.shared_params = {
        ...
        # Show happy face PNG on LCD at startup (after shared_params is set up)
        try:
            import lcd_faces
            lcd_faces.draw_happy_face()
            print("[LCD] Default happy face displayed at startup.")
        except Exception as e:
            print(f"[LCD] Could not display default happy face at startup: {e}")
    ...
    def update_tune(self, k, v):
        ...
        if k == "top_cam_arch_z":
            print(f"[DEBUG] top_cam_arch_z updated to {v}")
...
def start_brain_ui():
    ...
    if bool(getattr(config, "CRESTRON_SERVER_ENABLED", False)):
        print(f"[Crestron] TCP server enabled, listening on port {getattr(config, 'CRESTRON_PORT', 50005)}")
