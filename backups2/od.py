...
try:
    ...
    lcd_serial = spi(port=0, device=0, gpio_DC=25, gpio_RST=27, gpio_LIGHT=18)
    lcd_device = st7789(lcd_serial, width=320, height=240, rotate=0)
    image = Image.new("RGB", (lcd_device.width, lcd_device.height), (0, 0, 0))
    draw = ImageDraw.Draw(image)
    cx, cy = lcd_device.width // 2, lcd_device.height // 2
    face_radius = 80
    eye_radius = 10
    eye_offset_x = 30
    eye_offset_y = 25
    ...
    print("Displayed happy face on LCD")
except Exception as e:
    print(f"[LCD] Could not display happy face: {e}")
        ...
        if person:
            ...
            if landmarks:
                ...
                if (now - _last_move_time > config.MOVE_COOLDOWN
                    ...
                    print(f"[DEBUG] Tracking: base_arch_z={base_arch_z:.3f}, rz={rz:.3f}")
